...
const ProducerSession: FC<{ session: BusSession }> = ({ session }) => {
  ...
  // Drives the mascot's `face` prop so the mouth visibly animates.
  // Source of truth is the Rust speak_pump (edge-detected with ~400ms
  // hangover). Same `requestId` guards against stale events.
  ...
  useEffect(() => {
    ...
    // Subscribe to speaking-state events from the Rust speak_pump so
