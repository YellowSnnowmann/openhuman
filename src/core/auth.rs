...
pub async fn rpc_auth_middleware(req: axum::extract::Request, next: Next) -> Response {
    ...
    if bearer
        .strip_prefix("Bearer ")
        // NOTE: Plain string comparison is not timing-safe, but acceptable
        // here as the RPC server only binds to localhost by default and
        // the token is 256 bits, making brute-force timing attacks
        // impractical in this threat model.
