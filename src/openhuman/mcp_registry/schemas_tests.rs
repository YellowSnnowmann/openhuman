...
fn all_controller_schemas_covers_expected_methods() {
    ...
    // 17 mcp_clients (incl. update_env + registry_settings_get/set from #3039,
    // set_enabled from #3196, detect_auth + oauth_begin from #3495, and the
    // unified pre-install probe_auth) + 6 mcp_setup.
    ...
    let mcp_setup_count = schemas
        ...
        .count();
    assert_eq!(mcp_clients_count, 17);
    ...
    // The unified probe-based auth classifier.
    assert!(functions.contains(&"probe_auth"));
}

#[test]
fn all_registered_controllers_has_handler_per_schema() {
    let controllers = all_registered_controllers();
    assert_eq!(controllers.len(), 23);
}
